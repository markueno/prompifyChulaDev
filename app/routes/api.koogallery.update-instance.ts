...
export const action = async ({ request, context }: ActionFunctionArgs) => {
  try {
    ...
    const expireDate = new Date(
      expireTime.substring(0, 4) + '-' +  // year
      expireTime.substring(4, 6) + '-' +  // month
      expireTime.substring(6, 8) + 'T' +  // day
      expireTime.substring(8, 10) + ':' + // hour
      expireTime.substring(10, 12) + ':' + // minute
      expireTime.substring(12, 14) + 'Z'   // second
    );
