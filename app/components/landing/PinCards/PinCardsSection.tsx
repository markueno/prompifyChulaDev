...
export default function PinCardsSection() {
  ...
  return (
    <section ref={containerRef} className="pin-cards-section relative" style={{ background: '#f0e4d5' }}>
      <SectionBackground />

      {CARD_DATA.map(({ heading, description, Visual: sceneVisual }, index) => (
        <PinCard key={index} heading={heading} description={description} index={index}>
          <LazyVisual component={sceneVisual} rootMargin="300px" />
        </PinCard>
      ))}
    </section>
  );
