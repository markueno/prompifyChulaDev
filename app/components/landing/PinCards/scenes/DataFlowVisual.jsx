...
function drawBarChart(ctx, cx, cy, scale, alpha, colors) {
  ...
  bars.forEach((h, i) => {
    ...
    if (ctx.roundRect) {
      ctx.roundRect(bx, cy - bh, bW, bh, 2);
    } else {
      ctx.rect(bx, cy - bh, bW, bh);
    }
  ...
  useEffect(() => {
    ...
    const spawnRight = (burst = false) => {
      ...
      s.particles.push({
        ...
        t: 0,
        speed: burst ? 0.38 + Math.random() * 0.2 : 0.16 + Math.random() * 0.14,
      ...
      s.particles.forEach(p => {
        ...
        if (p.side === 'left') {
          ...
          const fadeFactor = Math.min(
            1,
            Math.max(0, (distFromLogo - LOGO_FADE_RADIUS * 0.4) / (LOGO_FADE_RADIUS * 0.8))
          );
          ...
          const fadeFactor = Math.min(
            1,
            Math.max(0, (distFromLogo - LOGO_FADE_RADIUS * 0.5) / (LOGO_FADE_RADIUS * 0.9))
          );
          ...
          if (p.type === 'bar') {
            drawBarChart(ctx, px, py, scale, alpha, p.colors);
          } else {
            drawLineChart(ctx, px, py, scale, alpha, p.colors);
          }
        }
      });
