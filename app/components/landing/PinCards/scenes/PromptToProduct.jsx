...
import ScaleToFit from '~/components/landing/PinCards/ScaleToFit';
...
export default function PromptToProduct() {
  ...
  const runAuto = async () => {
    ...
    for (const idx of AUTO_SEQUENCE) {
      if (cancelRef.current || userInteractedRef.current) {
        return;
      }
          ...
          {phase !== 'browser' ? (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 px-4 py-3" style={{ background: '#181818' }}>
                ...
                <div
                  className="flex-1 mx-3 h-5 rounded-md px-3 flex items-center"
                  style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)' }}
                >
                  <span className="text-[9px]" style={{ color: 'rgba(255,255,255,0.35)' }}>
                    app.prompify.ai/dashboard
                  </span>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between px-5 py-3" style={{ background: '#0d0d0d' }}>
                  ...
                  <div className="flex gap-5">
                    {['Overview', 'Analytics', 'Reports'].map(l => (
                      <div key={l} className="text-[9px]" style={{ color: 'rgba(255,255,255,0.35)' }}>
                        {l}
                      </div>
                    ))}
                  </div>
                  <div
                    className="rounded-md px-2 py-1 text-[8px] font-bold leading-none"
                    style={{ background: '#f97316', color: 'white', whiteSpace: 'nowrap' }}
                  >
                    Upgrade
                  </div>
                </div>
                ...
                <div className="p-5" style={{ background: '#f6f6f6' }}>
                  <div className="grid grid-cols-3 gap-2.5 mb-4">
                    {[
                      ...
                      >
                        <div className="text-[7px] mb-1 font-medium" style={{ color: 'rgba(35,23,16,0.55)' }}>
                          {l}
                        </div>
                        <div className="font-bold text-sm" style={{ color: '#231710' }}>
                          {v}
                        </div>
                        <div className="text-[8px] mt-0.5 font-semibold" style={{ color: '#10b981' }}>
                          ↑ {c}
                        </div>
                      </motion.div>
                    ))}
                  ...
                  >
                    <div className="text-[8px] font-semibold mb-2.5" style={{ color: 'rgba(0,0,0,0.35)' }}>
                      WEEKLY ACTIVITY
                    </div>
                    ...
                    <div className="text-[7px] text-center mt-1" style={{ color: 'rgba(0,0,0,0.25)' }}>
                      Click to restart
                    </div>
                  </motion.div>
                </div>
