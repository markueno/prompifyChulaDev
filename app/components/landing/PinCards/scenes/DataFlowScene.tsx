  ...
  useFrame((_, delta) => {
    ...
    charData.forEach((c, i) => {
      ...
      if (hasStarted.current) {
        ...
        if (charOffsets.current[i] < -4) {
          charOffsets.current[i] = 4;
        }
      }
    ...
    <>
      ...
      {rightParticles.map((p, i) => (
        <mesh
          key={`rp-${i}`}
          ref={el => {
            rightParticleRefs.current[i] = el;
          }}
          scale={0}
        >
          <sphereGeometry args={[0.05, 6, 6]} />
