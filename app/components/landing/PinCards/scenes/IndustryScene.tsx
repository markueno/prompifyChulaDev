...
export default function IndustryScene({ isActive }: SceneProps) {
  ...
  useFrame((_, delta) => {
    ...
    if (centerRef.current) {
      ...
      centerRef.current.scale.setScalar(centerRef.current.scale.x + (s - centerRef.current.scale.x) * delta * 3);
    }
    ...
    if (pulseRef.current) {
      ...
      pulseRef.current.scale.setScalar(pulseRef.current.scale.x + (targetS - pulseRef.current.scale.x) * delta * 3);
    }
    ...
    <>
      ...
      {ICONS.map((icon, i) => (
        ...
        >
          ...
          <Text position={[0, -0.6, 0]} fontSize={0.15} color="#ffffff" anchorX="center" anchorY="middle">
            {icon.label}
          </Text>
        </group>
      ))}
