  ...
  useFrame((_, delta) => {
    ...
    if (isActive) {
      for (let i = 0; i < particleTs.current.length; i++) {
        ...
        if (particleTs.current[i] >= 1) {
          particleTs.current[i] = 0;
        }
      }
    }
    ...
    particleTs.current.forEach((t, i) => {
      ...
      mesh.position.set(group.position.x * t, group.position.y * t, group.position.z * t);
    ...
    <>
      ...
      {NODES.map((node, i) => (
        ...
        >
          ...
          <Text position={[0, 0.55, 0]} fontSize={0.2} color="#ffffff" anchorX="center" anchorY="middle">
            {node.label}
          </Text>
        </group>
      ))}
