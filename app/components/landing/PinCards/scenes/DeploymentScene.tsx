...
const STAGES = ['SSO Integration', 'API Configuration', 'Compliance Check', 'Load Testing', 'Deploy'];
  ...
  useFrame((_, delta) => {
    ...
    if (!hasStarted.current) {
      ...
      if (progressLineRef.current) {
        progressLineRef.current.scale.y = 0;
      }
    ...
    <>
      ...
      {STAGES.map((stage, i) => (
        <group key={i}>
          ...
          <Text position={[0.8, NODE_Y[i], 0]} fontSize={0.18} color="#999999" anchorX="left" anchorY="middle">
            {stage}
          </Text>
