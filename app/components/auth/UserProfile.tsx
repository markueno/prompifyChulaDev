...
export function UserProfile({ user }: UserProfileProps) {
  ...
  const trigger = (
    <Button variant="ghost" className="flex items-center gap-2 text-white hover:text-[#fed7aa]">
      <div className="w-8 h-8 bg-[#f97316] rounded-full flex items-center justify-center text-white font-medium">
        {displayInitial}
      </div>
      <span className="hidden md:block text-sm font-medium text-white">{displayName || user.email}</span>
      <div className="i-ph:caret-down text-sm text-white/70" />
    </Button>
  );
